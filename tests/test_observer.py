...
def test_tracker():
    ...
    tracker.update_tracker("other_event", test_instance)
    assert tracker._iterations == 0
    assert tracker._previous_max is None
    assert tracker._previous_max_params is None

    tracker.update_tracker(Events.OPTIMIZATION_STEP, test_instance)
    ...
    tracker.update_tracker(Events.OPTIMIZATION_STEP, new_instance)
    ...
    tracker.update_tracker(Events.OPTIMIZATION_STEP, other_instance)
    assert tracker._iterations == 3
    assert tracker._previous_max == 7
    assert tracker._previous_max_params == [7, 7]

    tracker.time_metrics()
